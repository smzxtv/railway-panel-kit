...
## 一·五、手动添加入站节点（含端口原理说明）

正常情况下启动脚本会**自动创建** VLESS-WS 入站（日志出现 `[bootstrap] VLESS-WS 入站创建成功 ✓`）。若自动创建失败（日志提示 `WARN: 自动创建入站失败`），可在面板里手动添加，2 分钟搞定：

### 操作步骤（3X-UI 面板 → 入站列表 → + 添加入站）

| 标签页 | 字段 | 必须填的值 |
|---|---|---|
| 基础配置 | 备注 | 随意（如 `railway-node`） |
| 基础配置 | 协议 | `Vless` |
| 基础配置 | 地址 | 留空（监听所有 IP） |
| 基础配置 | **端口** | **`10085`**（原理见下） |
| 传输 | 传输方式 | **`ws`**，路径填 **`/ws`**（与 `VLESS_WS_PATH` 一致） |
| 安全 | 安全性 | **`无`**（不要选 TLS！TLS 由 Railway 边缘处理，这里选了会握手失败） |

其他字段（总流量 0 = 不限、到期时间空）保持默认即可。创建后客户端配置为：地址 = Railway 域名、端口 = `443`、UUID = 面板客户端里的 ID、传输 ws 路径 `/ws`、TLS 开启。

### 为什么端口必须是 10085？

这个端口**不是 Railway 或 3X-UI 官方规定的，而是本项目的内部约定**，定义在 `start.sh`：

```bash
export PORT=8080        # nginx 总入口，Railway 对外唯一端口
export PANEL_PORT=54321 # 3X-UI 面板（仅容器内）
export XRAY_PORT=10085  # Xray VLESS-WS 入站（仅容器内）
```

nginx 收到流量后按路径分流：`/{面板路径}/` 转发到 `127.0.0.1:54321`（面板），`/ws` 转发到 `127.0.0.1:10085`（Xray）。**面板里入站的监听端口必须与 `XRAY_PORT` 一致**，否则 nginx 转发无人接收 → 连接失败（表现为 `/ws` 返回 502）。

想换端口：在 Railway Variables 加 `XRAY_PORT=20000`，面板入站端口也填 `20000` 即可，nginx 会自动跟随该变量。
...
- **代理连不上（`/ws` 返回 502）**：90% 是入站没建成——查看日志有没有 `入站创建成功 ✓`；没有就按「一·五」手动添加入站（协议 vless、端口 `10085`、传输 ws 路径 `/ws`、安全无）。
- **日志里 `自动创建入站失败` / `面板登录失败`**：确认 `XUI_PASSWORD` 变量拼写正确且在**部署启动前**已存在（改完变量要点 Redeploy 才生效）。
- **手动建的入站重新部署后消失**：Railway 容器是临时的，正常现象。自动创建脚本会在下次部署时重建；在 Variables 里固定 `VLESS_UUID` 可保证客户端配置不变。
